...
namespace DroidExplorer.Core.UI.Components {
  public class ProcessInfoPidColumnSorter : IComparer {
    ...
    public int Compare ( object a, object b ) {
      if ( a is CpuProcessInfoListViewItem && b is CpuProcessInfoListViewItem ) {
        ...
        if ( lv.Sorting == SortOrder.Ascending ) {
          return ( a as CpuProcessInfoListViewItem ).ProcessInfo.PID.CompareTo ( ( b as CpuProcessInfoListViewItem ).ProcessInfo.PID );
        } else {
          return -( a as CpuProcessInfoListViewItem ).ProcessInfo.PID.CompareTo ( ( b as CpuProcessInfoListViewItem ).ProcessInfo.PID );
        }
      } else {
