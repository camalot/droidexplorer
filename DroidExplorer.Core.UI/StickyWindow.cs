...
namespace DroidExplorer.UI {
	public class StickyWindow : System.Windows.Forms.NativeWindow {
		...
		/// </summary>
		public static List<Form> GlobalStickyWindows { get; private set; }

		static StickyWindow () {
			GlobalStickyWindows = new List<Form> ();
		}
		...
		private bool DefaultMsgProcessor ( ref Message m ) {
			switch ( m.Msg ) {
				case NativeApi.WM_NCLBUTTONDOWN: {
						originalForm.Activate ();
						mousePoint.X = (short)NativeApi.Bit.LoWord ( (int)m.LParam );
						mousePoint.Y = (short)NativeApi.Bit.HiWord ( (int)m.LParam );
		...
		private bool OnNCLButtonDown ( int iHitTest, Point point ) {
			...
			switch ( iHitTest ) {
				case NativeApi.HTCAPTION: {	// request for move
						if ( stickOnMove ) {
		...
		private bool ResizeMsgProcessor ( ref Message m ) {
			...
			switch ( m.Msg ) {
				case NativeApi.WM_LBUTTONUP: {	// ok, resize finished !!!
						EndResize ();
						break;
					}
				case NativeApi.WM_MOUSEMOVE: {
						mousePoint.X = (short)NativeApi.Bit.LoWord ( (int)m.LParam );
						mousePoint.Y = (short)NativeApi.Bit.HiWord ( (int)m.LParam );
						Resize ( mousePoint );
						break;
				...
				case NativeApi.WM_KEYDOWN: {
						if ( (int)m.WParam == NativeApi.VK.VK_ESCAPE ) {
		...
		private bool MoveMsgProcessor ( ref Message m ) {	// internal message loop
			...
			switch ( m.Msg ) {
				case NativeApi.WM_LBUTTONUP: {	// ok, move finished !!!
						EndMove ();
						break;
					}
				case NativeApi.WM_MOUSEMOVE: {
