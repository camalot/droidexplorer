...
if( (Test-Path -Path Env:\CI_DEPLOY_CODEPLEX) -and ((Get-Item -Path Env:\CI_DEPLOY_CODEPLEX).Value) -eq $true ) {
	Invoke-MsBuild -Path "$env:APPVEYOR_BUILD_FOLDER\.appveyor\DeployCodePlex.msbuild" -MsBuildParameters "/verbosity:detailed /logger:""C:\Program Files\AppVeyor\BuildAgent\Appveyor.MSBuildLogger.dll"""
}
